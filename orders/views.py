...
class OrderListView(View):
    ALL_ORDER_TYPES       = ["IN_CART", "PURCHASE_INSTANT", "PURCHASE_CART", "PURCHASED"]
    ...
    def get(self, request):
        ...
        if order_type not in self.ALL_ORDER_TYPES:
            return JsonResponse({"status": "ORDER_TYPE_ERROR", "message": f'Order type should be one of {self.ALL_ORDER_TYPES}'}, status=400)
        ...
        for product_info in order_items:
            order_product = {
                ...
                "discounted_price"      : float(product_info.product.price * (1 - product_info.product.discount_ratio)),
                "thumbnail_url"         : product_info.product.thumbnail_url,
                "discount_ratio"        : float(product_info.product.discount_ratio),
                "total_discounted_price": float((product_info.product.price * (1 - product_info.product.discount_ratio)) * product_info.quantity),
    ...
    def post(self, request):
        try:
            ...
            if order_type not in self.POSTABLE_ORDER_TYPES:
                return JsonResponse({"status": "ORDER_TYPE_ERROR", "message": f'Order type should be one of {self.POSTABLE_ORDER_TYPES}'}, status=400)

            self.init_order_types(self.POSTABLE_ORDER_TYPES)
            ...
            for order_item in order_item_list:
                ...
                if OrderList.objects.filter(order__user=user, product=product, order__order_status__status=order_type).exists():
                    existing_order = OrderList.objects.filter(order__user=user, product=product, order__order_status__status=order_type)[0]
            ...
            return JsonResponse({"status": "SUCCESS", "data": {"order_list": self.get_order_info(order_type_list=self.POSTABLE_ORDER_TYPES, user=user)}}, status=200)
    ...
    def patch(self, request):
        try:
            ...
            if order_type not in self.PATCHEABLE_ORDER_TYPE:
                return JsonResponse({"status": "ORDER_TYPE_ERROR", "message": f'Order type should be one of {self.PATCHEABLE_ORDER_TYPE}'}, status=400)

            self.init_order_types(self.PATCHEABLE_ORDER_TYPE)
    ...
    def get_order_info(self, order_type_list=[], user=None):
        if user:
            order_list = OrderList.objects.filter(order__user=user, order__order_status__status__in=order_type_list)
        else:
